...
if [[ $param_parttype == 'efi' ]]; then
    run "Installing Ubuntu ${param_ubuntuversion} (~10 min)" \
        ...
        apt clean && \
        wget --header \\\"Authorization: token ${param_token}\\\" ${param_basebranch}/prebuilt/linux-image-5.4.42-custom+_2.1.0_amd64.deb && \
        wget --header \\\"Authorization: token ${param_token}\\\" ${param_basebranch}/prebuilt/linux-headers-5.4.42-custom+_2.1.0_amd64.deb && \
        dpkg -i linux-image-5.4.42-custom+_2.1.0_amd64.deb && \
        dpkg -i linux-headers-5.4.42-custom+_2.1.0_amd64.deb && \
        update-grub\"' && \
